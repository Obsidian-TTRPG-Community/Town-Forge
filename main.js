...
__export(main_exports, {
  ...
  LANDSCAPE_BASE_DISTANCE: () => LANDSCAPE_BASE_DISTANCE,
  SIZE_BASE_DISTANCE: () => SIZE_BASE_DISTANCE,
  TF_ICON_LIBRARY: () => TF_ICON_LIBRARY,
  TOWN_FORGE_TEMPLATES: () => TOWN_FORGE_TEMPLATES,
  TownForgePlugin: () => TownForgePlugin,
  TownForgeSettingTab: () => TownForgeSettingTab,
  VALID_SETTLEMENTS: () => VALID_SETTLEMENTS,
  VALID_TERRAINS: () => VALID_TERRAINS,
  default: () => main_default,
  extractSourceLink: () => extractSourceLink,
  frontmatterToConfigLines: () => frontmatterToConfigLines,
...
function parsePinTypesJson(json) {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    const noteType = String((_a = o.noteType) != null ? _a : "").trim();
    if (!noteType)
      return { error: `Item ${i}: "noteType" is required.` };
    let id = String((_b = o.id) != null ? _b : "").trim() || noteType.toLowerCase().replace(/\s+/g, "_");
    ...
    const min = Math.max(0, Math.floor(Number((_c = o.countMin) != null ? _c : 1)) || 0);
    const max = Math.max(min, Math.floor(Number((_d = o.countMax) != null ? _d : min)) || min);
    const t = {
      id,
      noteType,
      enabled: o.enabled !== false,
      icon: String((_e = o.icon) != null ? _e : "circle_black_city").trim() || "circle_black_city",
      layerName: String((_f = o.layerName) != null ? _f : "Places").trim() || "Places",
      placement,
      countMin: min,
      countMax: max,
      countMode,
    ...
    if ((placement === "anchored" || placement === "both") && typeof o.anchor === "string")
      t.anchor = o.anchor;
    if (nameMode === "js") {
      t.nameJs = String((_g = o.nameJs) != null ? _g : "");
    } else {
      const nl = (_h = o.nameLists) != null ? _h : {};
      t.nameLists = {
...
function anchorCentres(scene, anchor) {
  switch (anchor) {
    ...
    case "farm":
      return (scene.farms || []).map((f) => {
        var _a;
        return (_a = f.house) != null ? _a : f.polygon ? centroid(f.polygon) : null;
      }).filter((p) => !!p);
    default:
      return [];
  }
...
function buildMarkersFile(opts) {
  ...
  const markers = places.map((pl, i) => {
    ...
    return {
      ...
      link: (_a = pl.noteTitle) != null ? _a : pl.name,
...
function buildRoadNetwork(rng, scene, w, h, terrain, enabledEdges) {
  ...
  const finalize = (roads2, bridge2 = null) => {
    ...
    const ts = out.length ? (_a2 = out[0].town_site) != null ? _a2 : null : null;
  ...
  const primaryIndices = (_a = choosePrimaryPair(entries, scene, terrain, w, h)) != null ? _a : [0, 1];
...
function placeOutbuildings(rng, scene, townSite, zoneRadius, size, w, h) {
  ...
  const water = scene.water;
  const forests = (_a = scene.forests) != null ? _a : [];
  const ridges = (_b = scene.ridges) != null ? _b : [];
  const footprint = scene.footprint;
  const roads = (_c = scene.roads) != null ? _c : [];
  const wallRings = [];
  {
    const walls = scene.walls;
    if (walls) {
  ...
  const counts = (_d = OUTBUILDING_COUNTS[size]) != null ? _d : { mill: 0, inn: 0, stable: 0 };
  const placed = [];
  const outlanes = [];
  const beachDist = Math.min(w, h) * 0.02;
  ...
  }
  {
    ...
    for (const ob of placed) {
      ...
      const curved = (_f = (_e = buildCurved(ampBase)) != null ? _e : buildCurved(ampBase * 0.5)) != null ? _f : buildCurved(ampBase * 0.25);
      outlanes.push({ points: curved != null ? curved : [{ x: anchor.x, y: anchor.y }, end] });
    }
    if (dropped.size) {
...
function buildDock(rng, scene, townSite, zoneRadius, size, w, h, terrain) {
  ...
  const roads = (_a = scene.roads) != null ? _a : [];
  const bridgePts = [];
  for (const road of roads) {
    if (road.class === "bridge")
  ...
  const sc = (_b = scaleBySize[size]) != null ? _b : 0.8;
  ...
  const houses = (_c = scene.houses) != null ? _c : [];
  ...
  {
    const houses2 = (_d = scene.houses) != null ? _d : [];
    let near = false;
    const reach = 70;
...
function placeFarms(rng, scene, townSite, zoneRadius, size, w, h, houses, farmMul = 1) {
  ...
  const roadSegs = [];
  for (const road of (_c = scene.roads) != null ? _c : []) {
    let clr;
    if (road.class === "bridge")
      clr = 10;
      ...
      clr = 6;
    const pts = (_d = road.points) != null ? _d : [];
    for (let i = 0; i < pts.length - 1; i++) {
      roadSegs.push([pts[i].x, pts[i].y, pts[i + 1].x, pts[i + 1].y, clr]);
    }
  }
  for (const lane of (_e = scene.outlanes) != null ? _e : []) {
    const pts = (_f = lane.points) != null ? _f : [];
    for (let i = 0; i < pts.length - 1; i++) {
  ...
  const cap = Math.max(0, Math.round(((_g = FARM_CAP[size]) != null ? _g : 30) * farmMul));
  const farms = [];
...
function buildSettlement(rng, scene, w, h, size, terrain, overrides, cropSize) {
  ...
  if (townSite === null)
    return [];
  const zoneRadius = (_a = ZONE_RADIUS_BY_SIZE[size]) != null ? _a : 150;
  const armLength = ARM_LENGTH_BY_SIZE[size];
  const subRoadCount = (_b = SUB_ROAD_COUNT_BY_SIZE[size]) != null ? _b : 2;
  const houseTarget = (_c = HOUSE_TARGET_BY_SIZE[size]) != null ? _c : 15;
  ...
  if (isCity && footprint && footprint.length >= 3) {
    ...
    const roll = rng() < ((_d = wallChance[size]) != null ? _d : 0.7);
    const want = (_e = overrides == null ? void 0 : overrides.walls) != null ? _e : roll;
    if (want) {
      scene.walls = buildCityWalls(rng, scene, townSite, zoneRadius, size, w, h);
    }
  }
  let curHouses = (_f = scene.houses) != null ? _f : houses;
  if (isCity) {
  ...
  placeFarms(rng, scene, townSite, zoneRadius, size, w, h, curHouses, (_g = overrides == null ? void 0 : overrides.farmDensity) != null ? _g : 1);
  return (_h = scene.houses) != null ? _h : houses;
}
...
function placeHousesCity(rng, scene, w, h, townSite, coreRadius, armLength, targetCount, terrain, size) {
  ...
  const targetBlockArea = (_a = TARGET_BLOCK_AREA_BY_SIZE[size]) != null ? _a : 2e3;
  const rawBlocks = [];
  ...
  const parkCount = (_b = PARK_COUNT_BY_SIZE[size]) != null ? _b : 0;
...
function buildCastle(rng, scene, townSite, coreRadius, w, h, forced = false) {
  ...
  if (best === null) {
    if (insideFootprint({ x: fcx, y: fcy }, baileyR + 6)) {
      ...
      best = (_b = (_a = tryFind(baileyR + 6)) != null ? _a : tryFind(baileyR)) != null ? _b : tryFind(baileyR * 0.7);
      if (best === null) {
...
function placeLandmarks(rng, scene, townSite, coreRadius, size, terrain, w, h, overrides, cropSize) {
  ...
  const castleRoll = rng() < ((_a = castleChance[size]) != null ? _a : 0.5);
  if ((_b = overrides == null ? void 0 : overrides.castle) != null ? _b : castleRoll) {
    const castle = buildCastle(rng, scene, townSite, coreRadius, w, h, (overrides == null ? void 0 : overrides.castle) === true);
    if (castle) {
  ...
  const cathRoll = rng() < ((_c = cathChance[size]) != null ? _c : 0.4);
  if ((_d = overrides == null ? void 0 : overrides.cathedral) != null ? _d : cathRoll) {
    const cath = buildCathedral(rng, scene, townSite, coreRadius, w, h, avoid, (overrides == null ? void 0 : overrides.cathedral) === true);
    if (cath) {
      landmarks.push(cath);
  ...
  const barracksRoll = rng() < ((_e = barracksChance[size]) != null ? _e : 0.4);
  if ((_f = overrides == null ? void 0 : overrides.barracks) != null ? _f : barracksRoll) {
    const barracks = buildBarracks(rng, scene, townSite, coreRadius, w, h, avoid, (overrides == null ? void 0 : overrides.barracks) === true);
    if (barracks) {
  ...
  const slots = (_g = marketSlots[size]) != null ? _g : 1;
  const mChance = (_h = marketChance[size]) != null ? _h : 0.6;
  for (let i = 0; i < slots; i++) {
    const mRoll = rng() < mChance;
    const want = i === 0 ? (_i = overrides == null ? void 0 : overrides.market) != null ? _i : mRoll : (overrides == null ? void 0 : overrides.market) === false ? false : mRoll;
    if (!want)
      continue;
    const market = buildMarket(rng, scene, townSite, coreRadius, w, h, avoid, i === 0 && (overrides == null ? void 0 : overrides.market) === true);
    if (market) {
  ...
  if ((_j = overrides == null ? void 0 : overrides.tower) != null ? _j : towerRoll) {
    const tower = buildTower(rng, scene, townSite, coreRadius, w, h, (overrides == null ? void 0 : overrides.tower) === true, cropSize);
    if (tower) {
...
function makeLake(rng, noise, w, h, opts) {
  ...
  const roughness = opts.roughness;
  const octaves = opts.octaves;
  const lakeSize = (_a = opts.lakeSize) != null ? _a : 0.3;
...
function makeRiver(rng, noise, w, h, opts) {
  ...
  const roughness = opts.roughness;
  const octaves = opts.octaves;
  const riverWidth = (_a = opts.riverWidth) != null ? _a : 0.045;
  const edges = ["N", "E", "S", "W"];
...
function mountainsToRidges(mtns, avoidWater) {
  const ridges = [];
  for (const m of mtns) {
    const field = buildHeightField(m, 8, 44, avoidWater != null ? avoidWater : void 0);
...
function makeMountainSpine(rng, noise, w, h, opts, cropSize, anchor) {
  var _a, _b;
  const rangeLen = (_a = opts.rangeLen) != null ? _a : 0.65;
  const peakCount = Math.floor((_b = opts.peakCount) != null ? _b : 6);
...
function makeMountainSpineOverlay(rng, noise, w, h, opts, side, avoidWater, cropSize, anchor) {
  var _a;
  const peakCount = Math.floor((_a = opts.peakCount) != null ? _a : 6);
  if (peakCount <= 0)
    return { mountains: [], ridges: [] };
  const scale = 0.45 + peakCount / 6 * 0.55;
  const sxn = Math.min(w, h);
  const crop = cropSize != null ? cropSize : Math.min(w, h);
...
function generateFull(terrain, seedStr, opts) {
  ...
  const size = (_a = opts.size) != null ? _a : "town";
  const OUT = fullMode ? (_b = MAP_SIZE_BY_SIZE[size]) != null ? _b : 1e3 : 1e3;
  ...
  if (opts.showRoads !== false) {
    const enabled = (_c = opts.enabledEdges) != null ? _c : defaultEdges(rng, scene, w, h, terrain);
    const net = buildRoadNetwork(rng, scene, w, h, terrain, enabled);
    roads = net.roads;
    townSite = net.townSite;
  }
  if (terrain !== "mountain" && mtnSide && /[NESW]/.test(mtnSide)) {
    const anchor = townSite != null ? townSite : { x: w / 2, y: h / 2 };
  ...
  const forestMul = (_d = opts.overrides) == null ? void 0 : _d.forestDensity;
  if (forestMul !== void 0)
  ...
  const full = {
    ...
    townSite,
    size,
    houses: (_e = scene.houses) != null ? _e : houses,
    footprint: scene.footprint,
    parks: scene.parks,
    street_base: scene.street_base,
    walls: (_f = scene.walls) != null ? _f : null,
    landmarks: (_g = scene.landmarks) != null ? _g : [],
    farms: (_h = scene.farms) != null ? _h : [],
    outbuildings: (_i = scene.outbuildings) != null ? _i : [],
    outlanes: (_j = scene.outlanes) != null ? _j : [],
    dock: (_k = scene.dock) != null ? _k : null
  };
  if (fullMode) {
...
function drawBase(ctx, scene, w, h) {
  ...
  if (scene.mountains && scene.mountains.length) {
    for (const mtn of scene.mountains)
      drawContourMountain(ctx, mtn, (_a = scene.water) != null ? _a : void 0);
...
function drawCastle(ctx, c) {
  ...
  if (gate) {
    const ga = (_a = c.gateAngle) != null ? _a : 0;
...
var TownForgePreviewView = class extends import_obsidian.ItemView {
  constructor(leaf, getExportFolder, getTemplateFolder, getPinTypes, getOpenAfterExport, getGroupNotesByType, getEnableZoomMapExport, getShowTroubleshoot, getScaleMultiplier, getDistanceUnit) {
    ...
    this.getExportFolder = getExportFolder != null ? getExportFolder : () => "Maps";
    this.getTemplateFolder = getTemplateFolder != null ? getTemplateFolder : () => "Templates/TownForge";
    this.getPinTypes = getPinTypes != null ? getPinTypes : () => [];
    this.getOpenAfterExport = getOpenAfterExport != null ? getOpenAfterExport : () => true;
    this.getGroupNotesByType = getGroupNotesByType != null ? getGroupNotesByType : () => true;
    this.getEnableZoomMapExport = getEnableZoomMapExport != null ? getEnableZoomMapExport : () => false;
    this.getShowTroubleshoot = getShowTroubleshoot != null ? getShowTroubleshoot : () => false;
    this.getScaleMultiplier = getScaleMultiplier != null ? getScaleMultiplier : () => 1;
    this.getDistanceUnit = getDistanceUnit != null ? getDistanceUnit : () => "miles";
  }
  getViewType() {
  ...
  async readTemplate(buildingType) {
    ...
    for (const f of this.app.vault.getMarkdownFiles()) {
      const parent = ((_b = (_a = f.parent) == null ? void 0 : _a.path) != null ? _b : "").toLowerCase();
      if (parent !== tfLower)
        continue;
...
${body}`;
  ...
  fillTemplate(tpl, name, type, town, subtype, size) {
    return tpl.replace(/\{\{\s*name\s*\}\}/g, () => name).replace(/\{\{\s*type\s*\}\}/g, () => type).replace(/\{\{\s*subtype\s*\}\}/g, () => subtype != null ? subtype : "").replace(/\{\{\s*size\s*\}\}/g, () => size != null ? size : "").replace(/\{\{\s*town\s*\}\}/g, () => town);
  }
  ...
  async resolveNames(slots, town) {
    ...
    for (const slot of slots) {
      ...
      if (t.nameMode === "js" && t.nameJs && t.nameJs.trim()) {
        try {
          const r = await this.runNameHook(t.nameJs, { seed: slot.seed, town, type: t.noteType, index: slot.index, subtypes: (_a = t.subtypes) != null ? _a : [] });
          name = r.name;
          subtype = r.subtype;
        } catch (e) {
          name = "";
      ...
      if (!name) {
        const lists = (_b = t.nameLists) != null ? _b : { surnames: ["Oakhart"], adjectives: ["Old"], nouns: ["House"] };
  ...
  async runNameHook(body, ctx) {
    var _a, _b, _c, _d;
    const app = this.app;
    const rdm = (_b = (_a = app.plugins) == null ? void 0 : _a.plugins) == null ? void 0 : _b["randomness"];
    const api = rdm == null ? void 0 : rdm.api;
    ...
    if (out && typeof out === "object" && !Array.isArray(out)) {
      ...
      const subtype = String((_d = out.subtype) != null ? _d : "").trim();
      return { name, subtype };
    }
    return { name: String(out != null ? out : "").trim(), subtype: "" };
  }
  ...
  refresh() {
    ...
    const genSize = fullMode ? (_a = MAP_SIZE_BY_SIZE[s.settlement]) != null ? _a : 1e3 : 700;
    ...
    try {
      if (fullMode) {
        ...
        const baseDist = (_b = SIZE_BASE_DISTANCE[s.settlement]) != null ? _b : LANDSCAPE_BASE_DISTANCE;
  ...
  renderBlock(source, el, ctx) {
    ...
    try {
      const opts = {
        ...
        lakeSize: 0.3,
        rangeLen: 0.65,
        peakCount: (_a = config.peaks) != null ? _a : 6,
      ...
      if (fullMode) {
        ...
        renderFull(ctx2, full, genSize, genSize, config.terrain, mapDistance, distanceUnit, config.name);
  ...
  display() {
    ...
    if (this.plugin.tfSettings.enableZoomMapExport) {
      ...
      (async () => {
        ...
        try {
          portraitsReady = !!(api && api.portraits && await api.portraits.available());
        } catch (e) {
        ...
        try {
          ...
          if (tpLocal && typeof tpLocal.trigger_on_file_creation === "boolean")
            tpTrigger = tpLocal.trigger_on_file_creation;
          else if (tp && tp.settings && typeof tp.settings.trigger_on_file_creation === "boolean")
            tpTrigger = tp.settings.trigger_on_file_creation;
        } catch (e) {
          tpTrigger = void 0;
        }
        _tfSteps.empty();
        ...
        if (!tp)
          row(false, "Step 2 - install & enable Templater (scroll button on the right)");
        else if (tpTrigger === true)
          row(true, "Step 2 - Templater set to run on new files");
        else if (tpTrigger === false)
          row(false, "Step 2 - in Templater settings, turn ON \u201CTrigger Templater on new file creation\u201D");
        else
          row("warn", "Step 2 - make sure Templater\u2019s \u201CTrigger on new file creation\u201D is ON");
        if (hw)
          row(true, "Step 3 - Heraldry Weaver plugin installed & on");
        else if (hwInstalled)
          row(false, "Step 3 - enable the Heraldry Weaver plugin (the castle and guild templates draw their crests with it)");
        else
          row(false, "Step 3 - install & enable Heraldry Weaver (shield button on the right; draws the castle and guild crests)");
        row(portraitsReady ? true : "warn", portraitsReady ? "Step 4 - Fantasy Portrait Pack installed" : "Step 4 - in Randomness settings, click \u201CInstall Fantasy Portrait Pack\u201D (needed for Step 5; adds NPC faces)");
        if (canDetect)
          row(hubReady, hubReady ? "Step 5 - Fantasy Hub content installed, place generators ready" : "Step 5 - in Randomness settings, click \u201CInstall Fantasy Hub content\u201D (do Step 4 first)");
        else
          row("warn", "Step 5 - in Randomness settings, click \u201CInstall Fantasy Hub content\u201D (couldn\u2019t auto-check - older Randomness)");
        ...
        new import_obsidian2.Setting(containerEl).setName("Send pin icons to TTRPG Tools - Maps").setDesc('Detected TTRPG Tools - Maps ("' + (_mani.name || "Zoom Map") + `"). Write Town Forge's pin icons into its icon library so each pin shows its icon on the map. Safe to click again - it only adds or updates these icons and leaves your others alone.`).addButton(
          (b) => b.setButtonText("Send icons").setCta().onClick(async () => {
            try {
              ...
              new import_obsidian2.Notice("Town Forge: sending icons failed - " + (e && e.message ? e.message : String(e)), 8e3);
            }
          })
        );
      })();
...
var main_default = TownForgePlugin;
